...
```{r Train and test diurnal and linear models, echo=FALSE}
library(glmnet)
library(doParallel)
library(foreach)
library(neuralnet)
...
all.days <- foreach(days=1:6, 
                    .combine = 'c', .packages = c("xts","stats","glmnet","neuralnet")) %dopar% {
  # Days to train on
  training.days <- days
  ...
  for(f in horizon.steps) {
    ...
    testing.results <- do.call("rbind", lapply(1:(total.obs-2*f-training.obs+1), function(i) {
      ...
      # Calculate diurnal model training error
      ...
      # Train linear model when lambda=0 (initial parameter estimate)
      ...
      coefList <- coef(mod.adaptive, s='lambda.1se')
      coefList <- data.frame(colnames(obj.data)[coefList@i+1],coefList@x)
      names(coefList) <- c('var','val')
      coefList <- coefList[order(abs(coefList[,"val"]), decreasing=TRUE),]

      
      # Calculate lasso training error
      SSE <- mean((train.yy-pred.adapt)^2)
      SST <- mean((train.yy-mean(train.yy))^2)
      Rsqu.adapt <- 1-SSE/SST
      RMSE.adapt <- sqrt(SSE)
      SSE <- mean((train.yy-train.xx[,predict.col])^2)
      Rsqu.persistence <- 1-SSE/SST
      ...
      # if(i==1) {
        testing.results <- data.frame("Test.time"= test.index, 
                                      ...
                                      "LM.variables" = as.character(paste(coefList[,1], coefList[,2], collapse=" ")))
      # } else {
...
print(paste("Completed", obj, "at", Sys.time()))
...
```{r Which variables are significant?}
