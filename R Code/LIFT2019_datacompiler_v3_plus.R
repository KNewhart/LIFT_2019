...
# source("src/import_data.R")
# save(list=obj.list,file="data/ab3_objects_plus.RData")


load("data/ab3_objects_plus.RData")
...
for(x in obj.list) {
  predict.col <- which(colnames(get(x)) == "AB3.Z7.Ammonia.mg.N.L")
  if(length(predict.col) == 0) predict.col <- which(colnames(get(x)) == "NIA33391_AB3_AMMONIA")
  yy <- as.matrix(get(x)[,predict.col])
...
for(x in obj.list) {
  ...
  if(length(predict.col) == 0) predict.col <- which(colnames(lag.50) == "NIA33391_AB3_AMMONIA")
...
obj.list.lag <-as.character(paste("lag.50.",obj.list,sep=""))
