...
for(x in obj.list) {
  ...
  obj.p <- periodogram(na.omit(get(x))[,predict.col], log='yes')
  print(obj.p$freq[which(max(obj.p$spec) == obj.p$spec)]^-1*5/1440)
}
...
for(x in obj.list) {
  for(t in c(288, 288*2, 288*3, 288*4)) {
    ## Convert timestamps to runtime and project onto a unit circle 
    time.stamps <- difftime(index(get(x)), index(get(x))[1], units = "mins")
    time.stamps <- as.numeric(time.stamps) %% t # Cycles of 1 day are constructed (1440 min/day)
    time.stamps <- (time.stamps*360/t)*pi/180 # Cycles of minutes are converted to radians
    assign("x.t", cbind(time.stamps,get(x)))
    # }
    # ## Let's try another cycle size when I have more data!!!
    # i <- 1440*5
    # for(x in obj.list) {
    #   time.stamps <- difftime(index(get(x)), index(get(x))[1], units = "mins")
    #   time.stamps <- as.numeric(time.stamps) %% i # Cycles of 5 days are constructed (1440 min/day)
    #   time.stamps <- (time.stamps*360/i)*pi/180 # Cycles of minutes are converted to radians
    #   assign(x, cbind(time.stamps,get(x)))
    # }
    
    ## Diurnal model
    # for(x in obj.list) {
    predict.col <- which(colnames(get("x.t")) == "AB3.Z7.Ammonia.mg.N.L")
    
    yy <- as.matrix(get("x.t")[,predict.col])
    xx <- as.matrix(get("x.t")[,-predict.col])
    
    degrees <- 200
    r.sq.vals <- vector()
    # png(filename=paste0("v4/",x," diurnal acf.png"), width=480*2, height=480*2)
    par(mfrow=c(4,round(degrees/4)))
    for(i in 1:degrees) {
      ...
      # acf(resid(diurnal.mod),lag.max=2016, main = paste(x,i,"th degree diurnal"))
      # mtext(side=3, text=paste(round(summary(diurnal.mod)$adj.r.squared,2)))
      r.sq.vals <- c(r.sq.vals, summary(diurnal.mod)$adj.r.squared)
    }
    # dev.off()
    png(filename=paste0("v4/",x," - ",t*5/1440," days - ", degrees," deg diurnal rsq.png"))
    par(mfrow=c(1,1))
    plot(x=1:degrees, y=r.sq.vals, xlab="ith degree diurnal", 
         ylab="R-sq", pch=20, main=x.t)
    dev.off()
  }
  ...
  # fmla <- as.formula(paste("yy ~",paste(paste0("cos(",1:10," * xx[,1]) + sin(",1:10," * xx[,1])"),collapse=" + ")))
  # diurnal.mod <- lm(yy~cos(xx[,1])+sin(xx[,1])+cos(2*xx[,1])+sin(2*xx[,1]))
  # print(summary(diurnal.mod))
